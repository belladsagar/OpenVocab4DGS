...
from scene.base_readers import CameraInfo, SceneInfo, getNerfppNorm, fetchPly, get_PCA_Norm, get_Sphere_Norm
...
def readWaymoFullInfo(path, images='images', split_train=-1, split_test=-1, **kwargs):
    ...
    for i in tqdm(range(len(exts))):
        ...
        mask = None        
        cam_info = CameraInfo(
            uid=i, R=R, T=T, FovY=FovY, FovX=FovX, cx=cx, cy=cy, K=K,
